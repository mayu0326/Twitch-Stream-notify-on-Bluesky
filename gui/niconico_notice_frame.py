...
class NiconicoNoticeFrame(ctk.CTkFrame):
    def __init__(self, master=None):
        ...
        if os.path.exists(env_path):
            ...
            def get_env(key, default):
                return default
        notify_online = get_env('NOTIFY_ON_NICO_ONLINE', 'False').lower() == 'true'
        notify_newvideo = get_env('NOTIFY_ON_NICO_NEWVIDEO', 'False').lower() == 'true'
        tpl_online = get_env('BLUESKY_NICO_ONLINE_TEMPLATE_PATH', 'templates/nico_online_template.txt')
        tpl_newvideo = get_env('BLUESKY_NICO_NEW_VIDEO_TEMPLATE_PATH', 'templates/nico_new_video_template.txt')
        ...
        # UI
        ctk.CTkSwitch(self, text="ニコニコ生放送：開始通知を送信する", variable=self.var_online, font=DEFAULT_FONT, onvalue=True, offvalue=False).grid(row=0, column=0, sticky="w", pady=5)
        ctk.CTkSwitch(self, text="ニコニコ動画：動画投稿通知を送信する", variable=self.var_newvideo, font=DEFAULT_FONT, onvalue=True, offvalue=False).grid(row=1, column=0, sticky="w", pady=5)
        ...
        ctk.CTkButton(self, text="保存", command=self.save_nico_settings, font=DEFAULT_FONT, width=140).grid(row=9, column=1, sticky="w", pady=(10, 0))
    ...
    def save_nico_settings(self):
        ...
        for line in lines:
            if line.startswith('NOTIFY_ON_NICO_ONLINE='):
                new_lines.append(f'NOTIFY_ON_NICO_ONLINE={str(self.var_online.get())}\n')
                found_online = True
            elif line.startswith('NOTIFY_ON_NICO_NEWVIDEO='):
                new_lines.append(f'NOTIFY_ON_NICO_NEWVIDEO={str(self.var_newvideo.get())}\n')
                found_newvideo = True
            elif line.startswith('BLUESKY_NICO_ONLINE_TEMPLATE_PATH='):
                new_lines.append(f'BLUESKY_NICO_ONLINE_TEMPLATE_PATH={self._to_templates_relative(self.tpl_online.get())}\n')
                found_tpl_online = True
            elif line.startswith('BLUESKY_NICO_NEW_VIDEO_TEMPLATE_PATH='):
                new_lines.append(f'BLUESKY_NICO_NEW_VIDEO_TEMPLATE_PATH={self._to_templates_relative(self.tpl_newvideo.get())}\n')
        ...
        messagebox.showinfo('保存完了', 'ニコニコ通知設定を保存しました。')
