...
class NiconicoNoticeFrame(ttk.Frame):
    def __init__(self, master=None):
        ...
        self.var_nico_online = tk.BooleanVar(value=notify_online)
        self.var_nico_newvideo = tk.BooleanVar(value=notify_newvideo)
        ...
        ttk.Checkbutton(self, text="放送開始時に投稿", variable=self.var_nico_online,
                        style="Big.TCheckbutton").grid(row=0, column=0, sticky=tk.W, pady=5)
        ttk.Checkbutton(self, text="新着動画投稿時に投稿", variable=self.var_nico_newvideo,
        ...
        def update_tpl_nico_online_label():
            lbl_nico_tpl.config(text=os.path.basename(self.tpl_online.get()))
        lbl_nico_tpl = ttk.Label(self, text=os.path.basename(
        ...
        def update_img_label():
            lbl_nico_img.config(text=os.path.basename(self.img_path.get()))
        lbl_nico_img = ttk.Label(self, text=os.path.basename(
        ...
        def update_tpl_newvideo_label():
            lbl_nico_newvideo_tpl.config(
                text=os.path.basename(self.tpl_newvideo.get()))
        lbl_nico_newvideo_tpl = ttk.Label(self, text=os.path.basename(
        ...
        ttk.Button(self, text="保存", command=self.save_nico_settings, style="Big.TButton",
                   width=18).grid(row=10, column=1, sticky=tk.W, pady=(10, 0))
    ...
    def save_nico_settings(self):
        ...
        for line in lines:
            if line.startswith('NOTIFY_ON_NICONICO_ONLINE='):
                new_lines.append(
                    f'NOTIFY_ON_NICONICO_ONLINE={str(self.var_nico_online.get())}\n')
                found_online = True
            elif line.startswith('NOTIFY_ON_NICONICO_NEW_VIDEO='):
                new_lines.append(
                    f'NOTIFY_ON_NICONICO_NEW_VIDEO={str(self.var_nico_newvideo.get())}\n')
                found_newvideo = True
            elif line.startswith('BLUESKY_NICO_ONLINE_TEMPLATE_PATH='):
                new_lines.append(
                    f'BLUESKY_NICO_ONLINE_TEMPLATE_PATH={self._to_templates_relative(self.tpl_online.get())}\n')
                found_tpl_online = True
            elif line.startswith('BLUESKY_NICO_NEW_VIDEO_TEMPLATE_PATH='):
                new_lines.append(
                    f'BLUESKY_NICO_NEW_VIDEO_TEMPLATE_PATH={self._to_templates_relative(self.tpl_newvideo.get())}\n')
