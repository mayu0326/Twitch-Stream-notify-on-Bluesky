...
class TunnelConnection(tk.Frame):
    def __init__(self, master=None):
        ...
        headline.pack(fill=tk.X, padx=10, pady=(10, 0))
        # ラジオボタン
        ...
        for i, (label, value) in enumerate(services):
            ttk.Radiobutton(radio_frame, text=label, variable=self.selected_service, value=value, command=self.switch_service).grid(row=0, column=i, padx=8)
        # サービスごとのフレーム
