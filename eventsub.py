...
if TIMEZONE_NAME.lower() == "system": # Make comparison case-insensitive
    ...
    try:
        ...
        try:
            ...
            logger.warning(f"タイムゾーン '{TIMEZONE_NAME}' の処理中にエラーが発生しました ({e})。UTCにフォールバックします。")
            TIMEZONE = pytz.utc
...
TWITCH_BROADCASTER_ID = os.getenv("TWITCH_BROADCASTER_ID") # This might be updated by setup_broadcaster_id
WEBHOOK_SECRET = os.getenv("WEBHOOK_SECRET") # This is updated by rotate_secret_if_needed in main.py
...
def get_app_access_token(logger_to_use=None): # Allow passing logger
    ...
    if response.status_code == 200:
        ...
        current_logger.error(f"Twitchアクセストークン取得失敗: {response.status_code} - {response.text}")
        return None, 0
...
def get_broadcaster_id(username, logger_to_use=None): # Allow passing logger
    ...
    try:
        ...
        if not isinstance(data, list) or not data:
            current_logger.error(f"Twitch APIレスポンスにユーザーデータがありません (ユーザー名: {username})")
            raise ValueError(f"Twitch APIレスポンスにユーザーデータがありません (ユーザー名: {username})")
        return data[0]["id"]
...
def setup_broadcaster_id(logger_to_use=None): # Allow passing logger
    ...
    if TWITCH_BROADCASTER_ID is None or not TWITCH_BROADCASTER_ID.strip(): # Check if None or empty
        current_logger.critical("TWITCH_BROADCASTER_IDが設定されていません。アプリケーションは起動できません。")
    ...
    if not TWITCH_BROADCASTER_ID.isdigit():
        ...
        current_logger.info(f"TWITCH_BROADCASTER_ID '{original_username}' は数値ではありません。ユーザーIDの取得を試みます。")
        try:
            TWITCH_BROADCASTER_ID = get_broadcaster_id(original_username, logger_to_use=current_logger)
            current_logger.info(f"ユーザーID変換完了: {original_username} -> {TWITCH_BROADCASTER_ID}")
        except Exception as e:
            current_logger.critical(f"ユーザー名 '{original_username}' からユーザーIDへの変換に失敗しました: {e}", exc_info=True)
            raise
    else:
        current_logger.info(f"TWITCH_BROADCASTER_ID は既に数値形式です: {TWITCH_BROADCASTER_ID}")
...
# Signatureの検証
def verify_signature(request): # Assuming logger is available via app.logger in Flask context
    ...
    if delta > 300:  # 5分（300秒）を超えていれば拒否
        current_logger.warning(f"タイムスタンプが許容範囲外: {timestamp_str} (差分: {delta:.2f}秒)")
        return False
    ...
    digest = hmac.new(
        current_webhook_secret.encode('utf-8'), hmac_message.encode('utf-8'), hashlib.sha256
    ).hexdigest()
    expected_signature = f"sha256={digest}"

    if not hmac.compare_digest(signature, expected_signature):
        current_logger.warning(f"Webhook署名不一致。受信: {signature}, 期待値(計算結果): {expected_signature}")
        return False
...
def get_existing_eventsub_subscriptions(logger_to_use=None): # Allow passing logger
    current_logger = logger_to_use if logger_to_use else logger
    ...
    if response.status_code == 200:
        ...
        current_logger.error(f"既存サブスクリプション取得エラー: {response.status_code} - {response.text}")
        return []


# サブスクリプションの追加
def create_eventsub_subscription(event_type: str, logger_to_use=None): # Added event_type parameter and logger
    current_logger = logger_to_use if logger_to_use else logger
    current_audit_logger = logging.getLogger("AuditLogger") # Get audit logger instance

    # 既存サブスクリプション一覧を取得
    existing = get_existing_eventsub_subscriptions(logger_to_use=current_logger)
    for sub in existing:
        if (
            ...
            current_audit_logger.info(
                f"EventSubサブスクリプション作成: {event_type} (既に存在) 成功") # Updated audit log
            return {"status": "already exists", "id": sub.get("id"), "data": [sub]} # Return similar structure as new sub
    ...
    try:
        ...
        current_audit_logger.info(f"EventSubサブスクリプション作成: {event_type} 成功") # Updated audit log
        current_logger.info(f"EventSubサブスクリプション ({event_type}) を正常に作成しました。ID: {result.get('data', [{}])[0].get('id')}")
        return result
        ...
        current_audit_logger.warning(
            f"EventSubサブスクリプション作成: {event_type} 失敗: {reason}") # Updated audit log
        current_logger.error(f"EventSubサブスクリプション ({event_type}) 作成失敗: {reason}")
        ...
        current_audit_logger.warning(
            f"EventSubサブスクリプション作成: {event_type} 失敗: {reason}") # Updated audit log
        current_logger.error(f"EventSubサブスクリプション ({event_type}) 作成中に予期せぬエラー: {reason}", exc_info=True)
        return {"status": "error", "reason": reason}

# サブスクリプションの削除
def delete_eventsub_subscription(subscription_id, logger_to_use=None): # Allow passing logger
    current_logger = logger_to_use if logger_to_use else logger
...
# サブスクリプションの確認と削除の処理
def cleanup_eventsub_subscriptions(webhook_callback_url, logger_to_use=None): # Allow passing logger
    current_logger = logger_to_use if logger_to_use else logger
    ...
    for sub in subs:
        ...
        if sub_callback != webhook_callback_url or sub_status not in ["enabled", "webhook_callback_verification_pending"]:
            current_logger.info(f"サブスクリプションID {sub_id} (タイプ: {sub_type}, ステータス: {sub_status}, コールバック: {sub_callback}) は無効または古い設定のため削除します。")
            delete_eventsub_subscription(sub_id, logger_to_use=current_logger)
            deleted_count +=1
        else:
            current_logger.info(f"サブスクリプションID {sub_id} (タイプ: {sub_type}, ステータス: {sub_status}) は有効なため保持します。")
    
    current_logger.info(f"EventSubサブスクリプションのクリーンアップ完了。{deleted_count}件のサブスクリプションを削除しました。")
